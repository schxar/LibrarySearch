...
def forward_chat_history(history_id):
    """从10805转发请求到10806的/api/chat/history/<history_id>"""
    try:
        import requests
        # 构建目标URL
        target_url = f'http://localhost:10806/api/chat/history/{history_id}'
        ...
        app.logger.error(f"转发{history_id}到10806超时")
        return jsonify({'error': 'Service timeout'}), 504
    except requests.RequestException as e:
        app.logger.error(f"转发{history_id}请求失败: {str(e)}")
        return jsonify({'error': 'Forwarding failed'}), 502
    except Exception as e:
        app.logger.error(f"转发{history_id}异常: {str(e)}")
        return jsonify({'error': 'Internal error'}), 500
